export class WeatherService {
    ...
    async getAlertsForState(stateCode) {
        ...
        const formattedAlerts = features.map((feature) => this.formatAlert(feature));
        return `Active alerts for ${stateCode.toUpperCase()}:\n\n${formattedAlerts.join("\n")}`;
    }
    async getForecastForLocation(latitude, longitude) {
        // Get grid point data
        ...
        const formattedForecast = periods.map((period) => this.formatForecastPeriod(period));
